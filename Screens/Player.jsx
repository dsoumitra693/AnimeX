...
const Player = ({ route }) => {
  ...
  useEffect(() => {
    const filterByQuality = (sources) => {
      if (
        !(Object.keys(sources)?.length === 0 && sources.constructor === Object)
      ) {
        let videoUrl = sources?.find(
          (source) => source.quality == videoQuality
        ).url;
        return videoUrl;
      }
      return "";
  ...
  const nextEpisode = (currentEpisode) => {
    if (videoDetails.totalEpisodes != currentEpisode.number) {
      let _nextEpi = videoDetails.episodes.find(
        (video) => video.number == currentEpisode.number + 1
      );
      setEpisode(_nextEpi);
    }
  };
